...
public class RenderGuardian extends RenderLiving// Extender//<EntityGuardian>
{
    ...
    // boolean flag = entity.isInRangeToRender3d(d0, d1, d2) && (entity.ignoreFrustumCheck ||
    // p_147589_2_.isBoundingBoxInFrustum(entity.boundingBox) || entity.riddenByEntity == this.mc.thePlayer);
    // 1.8: in RenderGlobal.renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks)
    // boolean flag2 = this.renderManager.shouldRender(entity2, camera, d0, d1, d2) || entity2.riddenByEntity ==
    // this.mc.thePlayer;
     ...
     * that disregards the frustum completely when the beam is firing.
     */
     ...
     * if (this.RLExtender.shouldRender(livingEntity, camera, camX, camY, camZ))//(super.shouldRender(livingEntity,
        ...
        float partialTicks) // Renamed from 1.8's doRender()
    {
        ...
        if (targetedEntity != null && entityGuardian.getHealth() > 0) // Added this so that the beam vanishes
                                                                      // immediately when killing the Guardian
        {
            ...
            GL11.glRotatef((((float) Math.PI / 2F) + -f6) * (180F / (float) Math.PI), 0.0F, 1.0F, 0.0F); // Something to
                                                                                                         // do with the
                                                                                                         // axial
                                                                                                         // rotation of
                                                                                                         // the beam;
                                                                                                         // takes in
                                                                                                         // degrees
            GL11.glRotatef(f5 * (180F / (float) Math.PI), 1.0F, 0.0F, 0.0F); // Points the beam from Guardian to its
                                                                             // target; takes in degrees
            ...
            // As per Darian Stephens's recommendation, I've un-done the disabling at the beginning, before the pop
            // matrix:
