...
public class IntHashMap1122<V> {
    ...
    private transient IntHashMap1122.Entry<V>[] slots = new IntHashMap1122.Entry[16];
     ...
     */
    @Nullable
    final IntHashMap1122.Entry<V> lookupEntry(int hashEntry) {
        int i = computeHash(hashEntry);
    ...
    public void addKey(int hashEntry, V valueEntry) {
        ...
        for (IntHashMap1122.Entry<V> entry = this.slots[j]; entry != null; entry = entry.nextEntry) {
            if (entry.hashEntry == hashEntry) {
    ...
    private void grow(int p_76047_1_) {
        IntHashMap1122.Entry<V>[] entry = this.slots;
        int i = entry.length;

        if (i == 1073741824) {
            this.threshold = Integer.MAX_VALUE;
        } else {
            IntHashMap1122.Entry<V>[] entry1 = new IntHashMap1122.Entry[p_76047_1_];
            this.copyTo(entry1);
            this.slots = entry1;
     ...
     */
    private void copyTo(IntHashMap1122.Entry<V>[] p_76048_1_) {
        IntHashMap1122.Entry<V>[] entry = this.slots;
        int i = p_76048_1_.length;

        for (int j = 0; j < entry.length; ++j) {
            IntHashMap1122.Entry<V> entry1 = entry[j];

            if (entry1 != null) {
                entry[j] = null;

                while (true) {
                    IntHashMap1122.Entry<V> entry2 = entry1.nextEntry;
    ...
    public V removeObject(int p_76049_1_) {
        IntHashMap1122.Entry<V> entry = this.removeEntry(p_76049_1_);
     ...
     */
    @Nullable
    final IntHashMap1122.Entry<V> removeEntry(int p_76036_1_) {
        int i = computeHash(p_76036_1_);
        int j = getSlotIndex(i, this.slots.length);
        IntHashMap1122.Entry<V> entry = this.slots[j];
        IntHashMap1122.Entry<V> entry1;
        IntHashMap1122.Entry<V> entry2;
    ...
    private void insert(int p_76040_1_, int p_76040_2_, V p_76040_3_, int p_76040_4_) {
        IntHashMap1122.Entry<V> entry = this.slots[p_76040_4_];
        this.slots[p_76040_4_] = new IntHashMap1122.Entry(p_76040_1_, p_76040_2_, p_76040_3_, entry);
    ...
    static class Entry<V> {
        ...
        IntHashMap1122.Entry<V> nextEntry;
        /** The id of the hash slot computed from the hash */
        final int slotHash;

        Entry(int p_i1552_1_, int p_i1552_2_, V p_i1552_3_, IntHashMap1122.Entry<V> p_i1552_4_) {
        ...
        public final boolean equals(Object p_equals_1_) {
            if (!(p_equals_1_ instanceof IntHashMap1122.Entry)) {
                return false;
            } else {
                IntHashMap1122.Entry<V> entry = (IntHashMap1122.Entry) p_equals_1_;
        ...
        public final int hashCode() {
            return IntHashMap1122.computeHash(this.hashEntry);
        }
