...
public class VNComponentIglooPieces {
    ...
    abstract static class Feature extends StructureComponent {
        ...
        protected boolean func_74935_a(World worldIn, StructureBoundingBox structurebb, int yOffset) // In 1.9:
                ...
                for (int l = this.boundingBox.minZ; l <= this.boundingBox.maxZ; ++l) {
                    for (int i1 = this.boundingBox.minX; i1 <= this.boundingBox.maxX; ++i1) {
                        if (structurebb.isVecInside(i1, 64, l)) {
                            j += Math
                                .max(worldIn.getTopSolidOrLiquidBlock(i1, l), worldIn.provider.getAverageGroundLevel());
                            ++k;
                        }
                    }
                }
    ...
    public static class Igloo extends VNComponentIglooPieces.Feature {
        ...
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
            if (!this.func_74935_a(worldIn, structureBoundingBoxIn, -1)) // offsetToAverageGroundLevel()
            {
                ...
                this.placeBlockAtCurrentPosition(
                    worldIn,
                    Blocks.bed,
                    this.coordBaseMode + 8,
                    (this.coordBaseMode < 2 ? 6 : 2) + xOffset,
                    1,
                    5,
                    structureBoundingBoxIn); // Head
                this.placeBlockAtCurrentPosition(
                    worldIn,
                    Blocks.bed,
                    this.coordBaseMode,
                    ...
                    1,
                    4,
                    structureBoundingBoxIn); // Foot
                // Wall items
                this.placeBlockAtCurrentPosition(
                    worldIn,
                    Blocks.crafting_table,
                    0,
                    (this.coordBaseMode < 2 ? 2 : 6) + xOffset,
                    1,
                    5,
                    structureBoundingBoxIn);
                this.placeBlockAtCurrentPosition(
                ...
                this.placeBlockAtCurrentPosition(
                    worldIn,
                    Blocks.furnace,
                    (new int[] { 5, 3, 4, 2 })[this.coordBaseMode],
                    (this.coordBaseMode < 2 ? 2 : 6) + xOffset,
                    1,
                    3,
                    structureBoundingBoxIn);
                ...
                worldIn.setBlockMetadataWithNotify(
                    this.getXWithOffset((this.coordBaseMode < 2 ? 2 : 6) + xOffset, 3),
                    this.getYWithOffset(1),
                    this.getZWithOffset((this.coordBaseMode < 2 ? 2 : 6) + xOffset, 3),
                    (new int[] { 5, 3, 4, 2 })[this.coordBaseMode],
                    2);

                // Blank out the space next to the furnace to ensure it will face the right direction
                // this.placeBlockAtCurrentPosition(worldIn, Blocks.furnace, 0, (this.coordBaseMode<2?2:6)+xOffset, 1,
                // 3, structureBoundingBoxIn);
                // this.placeBlockAtCurrentPosition(worldIn, Blocks.air, 0, (this.coordBaseMode<2?2:6)+xOffset, 1, 4,
                // structureBoundingBoxIn);
                ...
                if (randomIn.nextDouble() < 0.5D) // Add an underground part!
                {
                    ...
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.trapdoor,
                        (new int[] { 8, 11, 9, 10 })[this.coordBaseMode],
                        4 + xOffset,
                        0,
                        5,
                        structureBoundingBoxIn);
                    ...
                    this.placeBlockAtCurrentPosition(
                        ...
                        basementCeilingY - 3,
                        0,
                        structureBoundingBoxIn); // Left of brewing stand
                    ...
                    for (int i : new int[] { -1, 1 }) {
                        this.placeBlockAtCurrentPosition(
                            worldIn,
                            Blocks.torch,
                            (new int[] { 4, 1, 3, 2 })[this.coordBaseMode],
                            4 + i + xOffset,
                            basementCeilingY - 1,
                            5,
                            structureBoundingBoxIn);
                    ...
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.torch,
                        (new int[] { 3, 2, 4, 1 })[this.coordBaseMode],
                        4 + xOffset,
                        basementCeilingY - 1,
                        1,
                        structureBoundingBoxIn);
                    ...
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.wall_sign,
                        (new int[] { 3, 4, 2, 5 })[this.coordBaseMode],
                        4 + xOffset,
                        basementCeilingY - 2,
                        1,
                        structureBoundingBoxIn);
                    ...
                    this.placeBlockAtCurrentPosition(
                        ...
                        (new int[] { 6, 5, 7, 4 })[this.coordBaseMode],
                        (this.coordBaseMode < 2 ? 6 : 2) + xOffset,
                        basementCeilingY - 3,
                        5,
                        structureBoundingBoxIn);
                    ...
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.spruce_stairs,
                        (new int[] { 7, 4, 6, 5 })[this.coordBaseMode],
                        (this.coordBaseMode < 2 ? 6 : 2) + xOffset,
                        basementCeilingY - 3,
                        3,
                        structureBoundingBoxIn);
                    ...
                    this.placeBlockAtCurrentPosition(
                        ...
                        basementCeilingY - 2,
                        5,
                        structureBoundingBoxIn); // 9 is "cactus"
                    ...
                    this.placeBlockAtCurrentPosition(
                        ...
                        basementCeilingY - 2,
                        0,
                        structureBoundingBoxIn); // Behind the sign
                    for (int i : new int[] { 3, 5 }) {
                        this.placeBlockAtCurrentPosition(
                            ...
                            basementCeilingY - 4,
                            -1,
                            structureBoundingBoxIn); // Floor of a cell
                    }
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.stonebrick,
                        2,
                        (this.coordBaseMode < 2 ? 1 : 7) + xOffset,
                        basementCeilingY - 2,
                        1,
                        structureBoundingBoxIn); // Above the mat
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.stonebrick,
                        2,
                        ...
                        basementCeilingY - 2,
                        4,
                        structureBoundingBoxIn); // Above the chest
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.stonebrick,
                        2,
                        ...
                        basementCeilingY - 2,
                        6,
                        structureBoundingBoxIn); // Behind the chest
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.stonebrick,
                        2,
                        ...
                        basementCeilingY - 2,
                        6,
                        structureBoundingBoxIn); // Right of brewing stand
                    this.placeBlockAtCurrentPosition(
                        worldIn,
                        Blocks.stonebrick,
                        2,
                        (this.coordBaseMode < 2 ? 7 : 1) + xOffset,
                        basementCeilingY - 2,
                        3,
                        structureBoundingBoxIn); // Left of brewing stand
                    ...
                    this.placeBlockAtCurrentPosition(
                        ...
                        Blocks.monster_egg,
                        3,
                        (this.coordBaseMode < 2 ? 3 : 5) + xOffset,
                        basementCeilingY - 3,
                        6,
                        structureBoundingBoxIn);
                    // Brick: in ceiling
                    ...
                    this.fillWithMetadataBlocks(
                        ...
                        5,
                        Blocks.ladder,
                        (new int[] { 2, 5, 3, 4 })[this.coordBaseMode],
                        Blocks.ladder,
                        (new int[] { 2, 5, 3, 4 })[this.coordBaseMode],
                        false);
                 ...
                 * worldIn.setBlock(i, j, k, Blocks.furnace,
                 * this.coordBaseMode==0?3:this.coordBaseMode==3?0:(this.coordBaseMode+2), 2);
                 * LogHelper.info("Furnace meta changed to " +
                 * (this.coordBaseMode==0?5:this.coordBaseMode==3?2:(this.coordBaseMode+2)) );
                 * }
                 * }
                 * }
                 * }
         ...
         */
        protected void basementTouchup(World worldIn, StructureBoundingBox structureBoundingBoxIn, Random randomIn,
            int minX, int maxX, int minZ, int maxZ, int coordBaseMode) {
                    ...
                    for (int z = minZ; z <= maxZ; z++) {
                        if (worldIn.getBlock(x, y, z) == Blocks.brewing_stand) {
                            ...
                            if (brewingStandTileEntity instanceof ISidedInventory) {
                                ((ISidedInventory) brewingStandTileEntity)
                                    .setInventorySlotContents(1, new ItemStack(Items.potionitem, 1, 16392));
                                worldIn.setTileEntity(x, y, z, brewingStandTileEntity);
                            }

                            // TileEntityBrewingStand brewingStandContents = new TileEntityBrewingStand();
                            // brewingStandContents.setInventorySlotContents(1, new ItemStack( Items.potionitem, 1,
                            // 16392));
                            ...
                            if (te instanceof IInventory) {
                                ...
                                WeightedRandomChestContent.generateChestContents(
                                    randomIn,
                                    iglooChestGoldapple.getItems(randomIn),
                                    (TileEntityChest) te,
                                    iglooChestGoldapple.getCount(randomIn));

                                ChestGenHooks iglooChest = ChestGenHooks.getInfo("iglooChest");
                                WeightedRandomChestContent.generateChestContents(
                                    randomIn,
                                    iglooChest.getItems(randomIn),
                                    (TileEntityChest) te,
                                    iglooChest.getCount(randomIn));
                            }
